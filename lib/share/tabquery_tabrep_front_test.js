...
import { domainSchemasTesttab } from './domain_schemas/domain_schemas_testtab.js';
...
async function genPayload({ filterSchema, parent, messagebox, appOptions }) {
    messagebox.clear();
    const tabrepFront = new TabrepFront({ domSchemasConfig: domainSchemasTesttab, parent, appOptions });
    const { tabrep } = tabrepFront;
    const { columns } = tabrep;
    const { tabExpression } = { tabExpression: 'testtab' };
