...
// Tabrep. Select.
////////////////////////////////////////////////////////////////////////////////
export function tabrepSelectFrontFunctionality({ tabMain, tabHead, tabBody, columns, messagebox, editingCell }) {
    const selectedFlag = { active: false };
    // Select cells area.
    let selectByMouseIsInProcess = false;
    const selectVector = { deltaX: 0, deltaY: 0 };
    let selectByKeydownIsInProcess = false;
    // Listeners.
    ...
    //////////////////////////////////////////////////////////////////////////////
    function onKeydownTable_copySelectAll(event) {
        trace({ typ: TRACE_TYP.listener, msg: 'table: keydown: copySelectAll: ' + event.code });
        if (editingCell.td)
            return; // if (event.target === editbox) return
    ...
    function onKeydownTable_moveFocus(event) {
        trace({ typ: TRACE_TYP.listener, msg: 'table: keydown: moveFocus: ' + event.code });
        if (event.shiftKey || event.ctrlKey || event.metaKey || event.altKey)
            return;
        if (!KEYS_TO_MOVE_CELL_FOCUS.has(event.code))
            return;
        try {
            moveFocusOnKeydownTable({ event, selectedFlag, tabMain, editingCell });
        }
        catch (err) {
            messagebox.message(err);
        }
    }
    function onKeydownTable_selectCells(event) {
        ...
        if (event.code === 'ArrowLeft')
            selectVector.deltaX -= 1;
        else if (event.code === 'ArrowRight')
            selectVector.deltaX += 1;
        else if (event.code === 'ArrowUp')
            selectVector.deltaY -= 1;
    ...
    function onMousedownTable_selectCells(event) {
        ...
        if (!(targetTd instanceof HTMLTableCellElement))
            return;
        ...
        if (!event.shiftKey)
            targetTd.focus();
        try {
            selectByTargetCell({ targetCell: targetTd, selectedFlag, columns, tabMain, tabHead });
        }
        catch (err) {
            messagebox.message(err);
        }
    }
    function onMouseoverTable_selectCells(event) {
        trace({ typ: TRACE_TYP.listener, msg: 'table: mouseover: selectCells' });
        if (!selectByMouseIsInProcess)
            return;
        let targetTd = event.target;
        if (targetTd instanceof HTMLDivElement && targetTd.classList.contains(TABREP_CSS_CLASSES.header)) {
            targetTd = targetTd.parentElement;
    ...
    function onMouseupTable_clearSelectFlag(event) {
...
// Select.
//////////////////////////////////////////////////////////////////////////////
export function tabrepSelectGetSelected(tabBody) {
    ...
    function getSelectedCellsArea() {
        ...
        for (const row of rows) {
            ...
            if (selectedCells.length > 0)
                selected.set(row, selectedCells);
        }
        return selected;
    }
}
function deselectAll({ selectedFlag }) {
    if (!selectedFlag.active)
        return;
...
function selectByShiftVector({ deltaX, deltaY, selectedFlag, columns, tabMain, tabHead }) {
    ...
    if (testRowIndex < firstRowIndex)
        deltaY = firstRowIndex - activeRowIndex;
    if (testRowIndex > lastRowIndex)
        deltaY = lastRowIndex - activeRowIndex;
    ...
    if (testCellIndex < 0)
        deltaX = -activeCellIndex;
    if (testCellIndex > targetRow.cells.length - 1)
        deltaX = targetRow.cells.length - 1 - activeCellIndex;
...
// Select cells:
function selectByTargetCell({ targetCell, selectedFlag, columns, tabMain, tabHead }) {
    const activeCell = document.activeElement;
    if (!(activeCell instanceof HTMLTableCellElement))
        return;
...
// Active.
//////////////////////////////////////////////////////////////////////////////
function moveFocusOnKeydownTable({ event, selectedFlag, tabMain, editingCell }) {
    const c = event.code;
    if (editingCell.td && (c === 'ArrowLeft' || c === 'ArrowUp' || c === 'ArrowRight' || c === 'ArrowDown'))
        return;
    if (editingCell.td)
    ...
    if (newColIndex < 0 || newRowIndex < 0)
        return; // newRowIndex < tabHead.rows.length
    const newActiveRow = tabMain.rows[newRowIndex];
    if (!(newActiveRow instanceof HTMLTableRowElement))
        return;
...
// Copy.
//////////////////////////////////////////////////////////////////////////////
function copyCells(tabBody) {
    const selected = tabrepSelectGetSelected(tabBody);
    const clipString = Array.from(selected.values())
