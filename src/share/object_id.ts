...
export class ObjectId {
  ...
  private [kIdBufBE]: ArrayBuffer

  /** @internal */
  private static increment: number = Math.round(Math.random() * 0xffffff)
   ...
   */
  constructor(inputId?: number | string | Uint8Array | ArrayBuffer) {
    if (inputId === undefined) {
      this[kIdBufBE] = ObjectId.generate()
    }

    else if (typeof inputId === 'number') {
      if (!Number.isInteger(inputId)) throw new Error()
      if (inputId < 0) throw new Error()

      const numBuf = num2bufBE(inputId)
      if (numBuf.byteLength > 12) throw new Error()
      ...
      for (let i = 0; i < numUint8Arr.length; i++) {
        ...
        if (numUint8 === undefined) throw new ErrorCustomImpossible('numUint8 === undefined')
      ...
      if (inputId.length === 24 && checkForHexRegExp.test(inputId)) {
        ...
        if (strUint8Arr.length !== 12) throw new Error()
      ...
      if (inputId.byteLength !== 12) throw new Error()
  ...
  static generate(): ArrayBuffer {
    ...
    if (procId_IdUint8Arr0 === undefined) throw new ErrorCustomImpossible('')
    if (procId_IdUint8Arr1 === undefined) throw new ErrorCustomImpossible('')
    if (procId_IdUint8Arr2 === undefined) throw new ErrorCustomImpossible('')
    if (procId_IdUint8Arr3 === undefined) throw new ErrorCustomImpossible('')
    if (procId_IdUint8Arr4 === undefined) throw new ErrorCustomImpossible('')
   ...
   */
  equals(otherId: number | string | Uint8Array | ArrayBuffer | ObjectId) {
    let otherIdUint8Arr: Uint8Array | null = null

    if (typeof otherId === 'number') {
      if (!Number.isInteger(otherId)) throw new Error()
      if (otherId < 0) throw new Error()

      const otherIdBuf = num2bufBE(otherId)
      if (otherIdBuf.byteLength > 12) throw new Error()
      ...
      if (otherId.length === 24 && checkForHexRegExp.test(otherId)) {
        ...
        if (otherIdUint8Arr.byteLength !== 12) throw new Error()
      }
    }

    else if (otherId instanceof Uint8Array) {
      if (otherId.byteLength !== 12) throw new Error()
    ...
    if (otherIdUint8Arr) {
      ...
      for (let i = 0; i < idUint8Arr.length; i++) {
        if (idUint8Arr[i] !== otherIdUint8Arr[i]) return false
      }
      return true
    }
