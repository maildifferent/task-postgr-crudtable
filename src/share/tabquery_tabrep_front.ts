...
import { DocumentUserCreateT, DocumentUserFilterI, DocumentUserKeyI, DocumentUserMainT, DocumentUserProjectT, domainSchemasUser } from './domain_schemas/domain_schemas_user.js'
...
async function main() {
  ...
  const filtersource = new FiltersourceFront<DocumentUserFilterI>({
...
async function genPayload(
  { filterSchema, parent, messagebox, appOptions }
    : { filterSchema: Partial<FilterSchemaI<DocumentUserFilterI, keyof DocumentUserFilterI>['filterSchema']> }
  ...
  const tabrepFront = new TabrepFront<
    ...
    DocumentUserFilterI,
    DocumentUserCreateT
  >({ domSchemasConfig: domainSchemasUser, parent, appOptions })
  const { tabrep } = tabrepFront
  const { columns } = tabrep
  const { tabExpression }: TabqueryTabExpressionI = { tabExpression: 'users' }
